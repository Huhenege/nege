...
const BookingManagement = () => {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            ...
            default:
                return { bg: '#f1f5f9', color: '#64748b', icon: AlertCircle, text: status };
        }
    };
    ...
    return (
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
            <div style={{ marginBottom: '2rem' }}>
                ...
                <p style={{ color: '#64748b' }}>Сургалтын захиалга болон төлбөрийн хяналт</p>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead>
                        <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Сургалт</th>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Хэрэглэгч</th>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Цаг</th>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Төлбөр</th>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Төлөв</th>
                            <th style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Үйлдэл</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            return (
                                <tr key={b.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <div style={{ fontWeight: '600', color: '#1e293b' }}>{b.trainingTitle}</div>
                                    </td>
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <div style={{ fontSize: '0.9rem', color: '#1e293b' }}>{b.userName || 'Зочин'}</div>
                                        <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{b.userEmail}</div>
                                    </td>
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.9rem', color: '#444' }}>
                                            <Calendar size={14} color="#64748b" /> {b.selectedDate}
                                        </div>
                                    ...
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <div style={{ fontWeight: '700', color: '#1e293b' }}>₮{Number(b.amount).toLocaleString()}</div>
                                    </td>
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <span style={{
                                            display: 'inline-flex',
                        ...
                        {filteredBookings.length === 0 && (
                            <tr>
                                <td colSpan="6" style={{ padding: '4rem', textAlign: 'center', color: '#94a3b8' }}>Захиалга олдсонгүй.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
